// ============================================
// Wagashi Spot Detail Component
// 和菓子屋詳細: ごちそうさまでした / 食べてみたい = チェックで記録
// ============================================
...
export default function WagashiDetail({ spot, onClose }: WagashiDetailProps) {
  ...
  const wagashiSpotMemos = useStore((state) => state.wagashiSpotMemos);
  ...
  const setWagashiSpotMemo = useStore((state) => state.setWagashiSpotMemo);
  const addWagashiSpotPhoto = useStore((state) => state.addWagashiSpotPhoto);
  const removeWagashiSpotPhoto = useStore((state) => state.removeWagashiSpotPhoto);
  const deleteCustomWagashiSpot = useStore((state) => state.deleteCustomWagashiSpot);
  const excludeWagashiSpot = useStore((state) => state.excludeWagashiSpot);
  const unexcludeWagashiSpot = useStore((state) => state.unexcludeWagashiSpot);

  const existingMemo = wagashiSpotMemos.find((m) => m.id === spot.id);
  ...
  useEffect(() => {
    const memo = wagashiSpotMemos.find((m) => m.id === spot.id);
    setNote(memo?.note || '');
    setRating(memo?.rating || 0);
    setShowCelebration(false);
  }, [spot.id, wagashiSpotMemos]);

  const isSpotTried = triedWagashiSpots.some((t) => t.id === spot.id);
  const isSpotWantToTry = wantToTryWagashiSpots.includes(spot.id);
  const isSpotExcluded = excludedWagashiSpots.includes(spot.id);
  const triedCount = triedWagashiSpots.length;

  useEffect(() => {
    if (note || rating > 0) setWagashiSpotMemo(spot.id, note, rating);
  }, [note, rating, spot.id, setWagashiSpotMemo]);
  ...
  const handleAddPhoto = useCallback(async () => {
    ...
    if (!result.canceled && result.assets[0]) addWagashiSpotPhoto(spot.id, result.assets[0].uri);
  }, [spot.id, addWagashiSpotPhoto]);

  const handleRemovePhoto = useCallback((uri: string) => removeWagashiSpotPhoto(spot.id, uri), [spot.id, removeWagashiSpotPhoto]);
  ...
  const handleSearchWeb = useCallback(() => {
    const query = [spot.prefecture, spot.name, '和菓子'].filter(Boolean).join(' ');
  ...
  const handleDeleteCustom = useCallback(() => {
    Alert.alert('この和菓子屋を削除しますか？', '自分で追加したお店を削除します。この操作は取り消せません。', [
      { text: 'キャンセル', style: 'cancel' },
      { text: '削除', style: 'destructive', onPress: () => { deleteCustomWagashiSpot(spot.id); onClose(); } },
    ]);
  }, [spot.id, deleteCustomWagashiSpot, onClose]);

  const handleToggleExclude = useCallback(() => {
    if (isSpotExcluded) unexcludeWagashiSpot(spot.id);
    else excludeWagashiSpot(spot.id);
  }, [spot.id, isSpotExcluded, excludeWagashiSpot, unexcludeWagashiSpot]);
  ...
  return (
    <View style={styles.content}>
      ...
      <View style={styles.tags}>
        ...
        <View style={styles.tag}>
          <Text style={styles.tagText}>{WAGASHI_GENRE_OPTIONS.find((o) => o.value === spot.genre)?.shortLabel ?? 'その他'}</Text>
        </View>
        {spot.isCustom && (
        ...
        {isSpotWantToTry && (
          <View style={[styles.tag, styles.tagWantToTry]}>
            ...
            <Text style={[styles.tagText, { color: BEER_COLORS.accent }]}>食べてみたい</Text>
          </View>
        )}
      </View>
      ...
      {spot.characteristics ? (
        <View style={styles.characteristicsSection}>
          <Text style={styles.characteristicsLabel}>このお店の特徴</Text>
      ...
      {showCelebration && (
        <View style={styles.celebrationBanner}>
          ...
          <Text style={styles.celebrationSubtext}>和菓子マップに追加しました</Text>
        </View>
      )}
      ...
      <Pressable style={[styles.triedButton, isSpotTried && styles.triedButtonActive]} onPress={handleToggleTried}>
        <Ionicons name={isSpotTried ? 'checkmark-circle' : 'restaurant-outline'} size={22} color={isSpotTried ? '#fff' : BEER_COLORS.accentSecondary} />
      ...
      <Pressable style={[styles.wantToTryButton, isSpotWantToTry && styles.wantToTryButtonActive]} onPress={handleToggleWantToTry}>
        ...
        <Text style={[styles.wantToTryButtonText, isSpotWantToTry && styles.wantToTryButtonTextActive]}>{isSpotWantToTry ? '食べてみたい！' : '食べてみたい'}</Text>
      </Pressable>
      ...
      {spot.isCustom && (
        <Pressable style={styles.deleteButton} onPress={handleDeleteCustom}>
          ...
          <Text style={styles.deleteButtonText}>この和菓子屋を削除</Text>
        </Pressable>
      )}
      <Text style={styles.source}>{spot.isCustom ? '自分で追加したお店' : '和菓子屋データ（サンプル含む）。店舗情報は変更されている場合があります'}</Text>
    </View>
  );
}
