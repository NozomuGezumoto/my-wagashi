// ============================================
// My Wagashi (和菓子) Data Loader
// Loads wagashi spots (shops, cafes, factories) for map pins
// ============================================

import { WagashiGeoJSON, WagashiFeature, WagashiPin, WagashiGenre } from '../types';
import { CustomWagashiSpot } from '../store/useStore';

import japanWagashiData from './japan_wagashi.json';
...
interface WagashiFeatureProps {
  id?: string;
  name: string;
  name_reading?: string;
  prefecture: string;
  address?: string;
  type: WagashiPin['type'];
  genre?: WagashiGenre;
  characteristics?: string;
  source?: string;
}

function featureToPin(feature: WagashiFeature): WagashiPin {
  const props = feature.properties as WagashiFeatureProps;
  const [lng, lat] = feature.geometry.coordinates;
  const name = props.name || '和菓子屋';
  return {
    id: props.id || `wagashi-${feature.geometry.coordinates.join('-')}`,
    lat,
    lng,
    name,
    nameReading: props.name_reading || name,
    type: props.type || 'shop',
    genre: props.genre || 'other',
...
export function customWagashiSpotToPin(spot: CustomWagashiSpot): WagashiPin {
  return {
    id: spot.id,
    lat: spot.lat,
    lng: spot.lng,
    name: spot.name,
    nameReading: spot.name,
    type: spot.type,
    genre: spot.genre,
    address: spot.address || '',
    prefecture: getPrefectureFromCoords(spot.lat, spot.lng),
    isCustom: true,
  };
}

export function getAllWagashiPins(): WagashiPin[] {
  const data = japanWagashiData as WagashiGeoJSON;
  return data.features.map((f) => featureToPin(f as WagashiFeature));
}

export function getWagashiSpotCount(): number {
  const data = japanWagashiData as WagashiGeoJSON;
